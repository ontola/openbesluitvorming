...
- The monitor follows `QUICKWIT_INDEX_ID` from the same `.env` the app reads, so a projection bump moves it along; override only via `WOOZI_MONITOR_QUICKWIT_INDEX_ID`. It used to carry a bare default and watched a frozen index for 18 days in August 2026.
- The served index is a local `file://` index on the dedicated volume, created with `commit_timeout_secs: 60`. There is no searcher split cache and the `quickwit.yaml` deliberately has no `split_cache` section: the cache wraps `file://` storage too and would copy the index onto itself. Everything the cache used to need -- a janitor, a purge, a phantom check -- is gone; the history is in `docs/search-performance-quickwit-s3.md` and `docs/handover-split-cache-and-reindex.md`. A new index is created by pointing the workers at a new id with `WORKER_QUICKWIT_INDEX_URI` and `WORKER_QUICKWIT_COMMIT_TIMEOUT_SECS` set (`deployment.md`, "Creating an index somewhere else than the node default").
