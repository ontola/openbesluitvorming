...
Deno.test("GO + Allmanak normalizers produce canonical ids", async () => {
  ...
  assert(
    meeting.committee === "committee:gemeenteoplossingen:gemeente:goirle:10",
    "committee id",
  );
  assert(documents.some((doc) => doc.id.endsWith(":500")), "includes meeting-level document");
  assert(documents.some((doc) => doc.id.endsWith(":501")), "includes agenda document");
  assert(
  ...
  const fixturePeople = ((rawPersons as any)[0].functies[0].functie.medewerkers.map((m: any) =>
    m.persoon
  )) as never;
  ...
  assert(persons[0].id.startsWith("person:allmanak:gemeente:goirle:"), "person id supplier allmanak");
